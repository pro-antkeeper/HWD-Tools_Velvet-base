...
void CrystalAura::onTick(C_GameMode* gm) {
	...
	if (delay == delay2 + 1) {
		if (autoplace || g_Data.getLocalPlayer()->getSelectedItemId() == 637) {  // endcrystal
			for (auto& i : targetList)
				if (multiaura)
					CPlace(gm, &i->getPos()->add(0, -1, 0));  // i‚ðŽg‚¤‚±‚Æ‚É‚æ‚Á‚Ä•¡”‚Ì‘ŠŽè‚ÉCrystalAura‚ð‚·‚é‚±‚Æ‚ª‚Å‚«‚Ü‚·
				else
					CPlace(gm, &targetList[0]->getPos()->add(0, -1, 0));  // ƒVƒ“ƒvƒ‹‚È’P‘Ì‚Ìƒ^[ƒQƒbƒg‚ÉƒNƒŠƒXƒ^ƒ‹‚ð‚¨‚«‚Ü‚·B
			if (targetList.empty()) getRenderPositon = vec3_ti(0, 0, 0);    //‚±‚ê‚Í0,0,0‚É‚ ‚éê‡‚ÍƒŒƒ“ƒ_ƒŠƒ“ƒO‚µ‚È‚¢‚Ì‚ÅAƒŒƒ“ƒ_ƒŠƒ“ƒO‚ð‚µ‚È‚¢‚æ‚¤A0,0,0‚É‚µ‚Ä‚¢‚Ü‚·
		}
		return;
	}
