...
using namespace std;
Scaffold::Scaffold() : IModule(0, Category::PLAYER, "Places blocks under you ~Made by #Team GameSense")
{
	registerEnumSetting("Mode", &extendType, 0);
	extendType.addEntry(EnumEntry("Packet", 0));
	extendType.addEntry(EnumEntry("Celsius", 1));
	...
	rotations.addEntry(EnumEntry("ZipLine", 6));
	...
	holdType.addEntry(EnumEntry("Switch", 0));
	holdType.addEntry(EnumEntry("Spoof", 1));
	holdType.addEntry(EnumEntry("FakeSpoof", 2));
	holdType.addEntry(EnumEntry("HotbarSpoof", 3));
	holdType.addEntry(EnumEntry("None", 4));
	registerEnumSetting("Place", &diagType, 0);
	diagType.addEntry(EnumEntry("None", 0));
	diagType.addEntry(EnumEntry("UnderBlock", 1));
	diagType.addEntry(EnumEntry("PlaceDelay", 2));
	diagType.addEntry(EnumEntry("Beta", 3));
	diagType.addEntry(EnumEntry("NoSpeed", 4));
	diagType.addEntry(EnumEntry("UpBlock", 5));
	registerEnumSetting("BlockCounter", &blocleft, 0);
	blocleft.addEntry(EnumEntry("None", 0));
	blocleft.addEntry(EnumEntry("Underlined", 1));
	...
	downwards.addEntry(EnumEntry("None", 0));
	downwards.addEntry(EnumEntry("Vanilla", 1));
...
{
	if (rotations.selected <= 9)
		name = string("Scaffold ") + string(GRAY) + rotations.GetEntry(rotations.selected).GetName();
	else
...
void Scaffold::onTick(C_GameMode* gm)
{
	...
	if (diagType.selected == 2 && !jumping && velocityxz >= 0.01 && player->velocity.y <= 0.01)
		groundtime2++;
	else
		groundtime2 = 0;
	...
	// UpPlace
	if (diagType.selected == 3 || diagType.selected == 4)
	{
		if ((player->velocity.x > 0.06 || player->velocity.x < -0.06) && (player->velocity.z > 0.06 || player->velocity.z < -0.06) && diagType.selected == 3)
		{
			lockY = false;
			blockBelowY = g_Data.getLocalPlayer()->eyePos0; // Block below the player
			blockBelowY.y -= g_Data.getLocalPlayer()->height;
		...
		else if (diagType.selected == 3)
			lockY = true;
	}

	// Build Down Block
	bool downwardPlaced = false;
	if (sneaking && downwards.selected == 1)
	{
		downwardPlaced = true;
	...
	// Celsius
	if (extendType.selected == 1)
	{
		if (((diagType.selected == 0 || diagType.selected == 1 || diagType.selected == 3 || diagType.selected == 4 || diagType.selected == 5) && !jumping && velocityxz >= 0.01) || groundtime >= 10 || groundtime2 >= 10 || (diagType.selected == 2 && !jumping && velocityxz >= 0.01 && telly2))
		{
		...
		{
			if (groundtime2 >= 5 || groundtime >= 5 || velocityxz <= 0.01 || (diagType.selected == 0 || diagType.selected == 1 || diagType.selected == 3 || diagType.selected == 4 || diagType.selected == 5) && player->onGround || telly2)
			{
				if (isBlockReplacable(blockBelow))
					predictBlock(blockBelow);
				else
	...
	if (holdType.selected > 0)
	{
		if (holdType.selected == 1)
		{
...
void Scaffold::onPreRender(C_MinecraftUIRenderContext* renderCtx) //visuals are all fucked up :sob:
{
	auto player = g_Data.getLocalPlayer();
	if (player == nullptr)
		return;
	...
	// Zephyr
	{
		...
		string text = "   Blocks Left: " + to_string(totalCount); //swag
		...
		DrawUtils::drawText(textPosZephyr, &text, MC_Color(255, 255, 255), 1.f, 1.f); //missing shit
	...
	// Packet+
	{
		...
		DrawUtils::drawText(vec2_t(textPos.x - 4, textPos.y), &count, MC_Color(255, 255, 255), 1.f, 1.f);
	}
	...
	// OldExistent
	{
		...
		DrawUtils::drawText(vec2_t(textPos.x - 4, textPos.y), &count, color, 1.f, 1.f);
	}
}
...
bool Scaffold::selectBlock()
{
	...
	if (holdType.selected == 1 || holdType.selected == 2)
	{
		for (int n = 0; n < 9; n++)
		{
...
bool Scaffold::isUsefulBlock(C_ItemStack* itemStack)
{
	...
	vector<string> uselessNames = { "cake", "ladder", "tnt", "lever", "loom", "scaffolding", "web", "sand", "gravel", "dragon_egg", "anvil", "bamboo", "bed"};
...
void Scaffold::onDisable()
{
	...
	if (firstlock)
		...
		lockY = false;
	if (tower.selected == 1 && foundBlock && jumping)
