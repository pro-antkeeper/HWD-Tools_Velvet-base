...
bool TargetUtil::isValidTarget(C_Entity* ent) {
	...
	// Anarchy server dont need Antibot LOL - trung
	// unfortunately its now semi minigame - merci
	// might aswell ig - ant
	if (antibot->isEnabled()) {
		if (antibot->mode.selected == 0) {  // Basic
			...
			if (!antibot->isHitboxCheckEnabled()) if ((ent->height < 1.5f || ent->width < 0.49f || ent->height > 2.1f || ent->width > 0.9f)) return false; //no hitbox fix??
			...
			if (antibot->isHitboxCheckEnabled()) if ((ent->height < 1.5f || ent->width < 0.49f || ent->height > 2.1f || ent->width > 0.9f)) return false;
