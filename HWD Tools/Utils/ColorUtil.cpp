...
/*MC_Color ColorUtil::RGBWave(int red, int green, int blue, int red2, int green2, int blue2, long index)
{
	...
	float r, g, b = 0;
	Utils::ColorConvertHSVtoRGB(hue, interface->saturation, 1, r, g, b);
...
MC_Color ColorUtil::interfaceColor(int index)
{
	...
	{
	case 0:
		color = ColorUtil::rainbowColor(8, i->saturation, 1.F, -index * 4.f);
		break; /* Rainbow */
	case 1:
...
MC_Color ColorUtil::interfaceColortwo(int index)
{
	...
	{
	/*case 0:
		color = ColorUtil::rainbowColortwo(8, i->saturation, 1.F, -index * 4.f);
		break; /* Rainbow */
	/*case 1:
...
/*MC_Color ColorUtil::rainbowColorthree(float seconds, float saturation, float brightness, long index) //notifications stuff right?
{
	float hue6 = (((TimerUtil::getCurrentMs() + index) % (int)(6000)) / (float)(6000));
	float r, g, b = 0;
